...
목록 화면의 이 값은 **모델이 스스로 매긴 확신도**다. 측정된 정확도가 아니다.

Whisper 계열이 세그먼트마다 내는 평균 로그확률(`avg_logprob`)에 `exp` 를 취해 0~1 로
옮기고, 무음 확률(`no_speech_prob`)만큼 깎은 값이다. Job 단위 값은 세그먼트를 **길이로
가중해** 평균한다 — 0.5초짜리 감탄사와 20초짜리 설명이 같은 무게를 가지면 전체 인상이
왜곡된다.

**확신에 차서 틀리는 경우도 있다.** 이 값이 높다고 맞다는 뜻이 아니며, 낮으면 사람이
확인해 볼 구간이라는 신호일 뿐이다. 그래서 화면 제목도 "변환 정확도(추정)"이다.

다만 실전에서 유용한 신호가 하나 있다. Whisper 계열은 끝부분 무음에서 "자막제공자"
같은 문구를 만들어 내는 실패 모드가 있는데, 그 구간의 확신도가 눈에 띄게 낮게 나온다.

```
[  0.0-  7.0] conf=0.829  네, 고객센터 김민수입니다. 무엇을 도와드릴까요?
[ 30.0- 32.0] conf=0.537  자막제공자          <- 환각 구간
```

확신도를 주지 않는 엔진(Mock 등)과 이 기능 이전에 만들어진 Job 은 값이 비어 있으며,
목록에 `-` 로 표시된다. **0% 로 채우지 않는다** — "확신도 0"으로 오해되기 때문이다.

---

## 4.7 라이브 캡션 재생

전사 결과를 한 줄씩 드러내는 **표시 효과**다. 상담 상세와 상담 목록 화면에서 결과가
처음 뜰 때 한 번 돌고, `캡션 재생` 버튼으로 다시 볼 수 있다.
