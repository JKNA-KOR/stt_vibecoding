...
정상 파일 5개(WAV/MP3/FLAC/M4A/OGG)와 **거부되어야 하는 파일 3개**(확장자 위장,
빈 파일, 허용되지 않는 확장자)를 만든다. 정상 경로만 확인하면 검증이 실제로 걸리는지
알 수 없으므로 거부 케이스가 함께 있다.

스크립트는 결정론적이라 같은 `--seed` 로 같은 파일이 나온다. 그래서 산출물은
커밋하지 않는다 (`/samples/` 는 .gitignore 대상).

> **이 파일들에는 사람의 발화가 없다.** 말소리를 흉내 낸 신호이므로 Mock 엔진과
> 업로드 검증 확인에만 쓴다. 실제 모델에 넣어도 의미 있는 텍스트는 나오지 않으며,
> 전사 품질 확인에는 발화가 담긴 음성이 따로 필요하다.
